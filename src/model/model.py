...
class PSFVolumeModule(LightningModule):
    ...
    def __init__(
        ...
        self.bound_xy, self.bound_xy_max = self.calc_psfVbound(meta_data['umtx'], model_cfg.patchsize_uv, depth_range, meta_data['focal_distance'])
    ...
    def calc_psfVbound(self, K_mat, patchsize_uv, depth_range, focal_distance):
        '''
            patchsize_uv : patch size in uv coord of observed image
        ...
        base_xy_size = max(patchsize_uv / fx, patchsize_uv / fy)
    ...
    def model_step(self, batch):
        ...
        patchSize = clean_img.shape[2]
        kernel_size = int(self.hparams.meta_data.patchRatio * self.bound_xy_max)
        ...
        # create valid mask
        with torch.no_grad():
            ones_kernel = torch.ones((clean_img.shape[1], clean_img.shape[1], kernel_size, kernel_size), device=clean_img.device)
            mask = F.conv2d(clean_img, weight=ones_kernel, stride=1, padding=kernel_size // 2, dilation=1) > 0
            mask_gradient = rearrange((mask.float() * batch['mask']).sum(dim=1), 'b h w -> b (h w)') > 0
            gradient_left = gradient_left * batch['weight']
            gradient_right = gradient_right * batch['weight']
            output.update({'mask_gradient': rearrange(mask_gradient, 'b (h w) -> b h w', h=patchSize, w=patchSize)})
            ...
            if self.diffusion_model is not None:
                ...
                blur_left_img = F.interpolate(left_diff.pred_x_start, size=patchSize, mode='bilinear', align_corners=True)
                blur_right_img = F.interpolate(right_diff.pred_x_start, size=patchSize, mode='bilinear', align_corners=True)
            else:
        ...
        " learn to blur "
        # unfolding
        uvcoord_unfold = unfolding(batch['uv_coord'], kernel_size=kernel_size)  # already distorted uv coordinates
        pts3d_unfold = unfolding(batch['3d_coord'], kernel_size=kernel_size)  # 3d scene points
        ...
        if self.hparams.model_cfg.use_symmetric_loss:
            ...
            xy_coords = torch.linspace(-1/2*self.bound_xy, 1/2*self.bound_xy, patchSize).to(clean_img.device)
            xyz_coords = torch.stack(torch.meshgrid(xy_coords, xy_coords, depths, indexing='xy'), dim=-1)  # [patch_size, patch_size, level, 3]
            xyz_coords_ = rearrange(xyz_coords, 'h w l c -> (h w l) c')
            feat_dp = self.blur_volume['featnet'](xyz_coords_)
            coords = torch.cat([torch.zeros_like(xyz_coords_[..., :2]), xyz_coords_], dim=1)
    
            if 'embedder' in self.blur_volume:
                coords = self.blur_volume['embedder'](coords)
                
            kernel_w = self.blur_volume['mlpnet'](torch.cat([coords, feat_dp], dim=1))
            kernel_w = rearrange(kernel_w, '(h w l) c -> h w l c', h=patchSize, w=patchSize)
