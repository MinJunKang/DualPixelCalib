...
def read_observations(dirpath, data_type='calib', scale=0.5):
    ...
    if data_type == 'calib':
        ...
        for subdir in tqdm(subdirs):
            files_left = sorted([path for path in (subdir / 'LEFT').glob('*') if path.suffixes[0] in ['.png', '.jpg', '.jpeg', '.JPEG', '.TIF', '.pgm']])
            files_right = sorted([path for path in (subdir / 'RIGHT').glob('*') if path.suffixes[0] in ['.png', '.jpg', '.jpeg', '.JPEG', '.TIF', '.pgm']])
