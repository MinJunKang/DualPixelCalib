...
class CalibBoard(object):
    ...
    def extractPatches(self, observations: dict, calib_results: dict):
        ...
        # initialize data
        mask_patch_valid = []
        training_data = {'clean_patch': [], 'noisy_patch': [], 'lpatch': [], 'rpatch': [], 'patch_3d': [], 'patch_uv': [], 'patchSize_px': [], 'patchSize_mm': [], 'rvec': [], 'tvec': []}
        for psfboard in self.psf_board:
            circenters = psfboard['cir2d'].reshape(-1, 3)
            patchSize_px = round((psfboard['size_px'] / psfboard['num_grid'] * self.opts.calib.psfboard.patch_size_area) / self.opts.calib.psfboard.patch_size_mul) * self.opts.calib.psfboard.patch_size_mul
            patchSize_mm = self.opts.calib.intboard.board_size_mm / (psfboard['size_px'] / psfboard['num_grid']) * patchSize_px
            training_data['patchSize_px'].append(patchSize_px)
            training_data['patchSize_mm'].append(patchSize_mm)
            training_data['clean_patch'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 3], dtype=np.float32))
            training_data['noisy_patch'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 3], dtype=np.float32))
            training_data['lpatch'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 3], dtype=np.float32))
            training_data['rpatch'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 3], dtype=np.float32))
            training_data['patch_3d'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 3], dtype=np.float32))
            training_data['patch_uv'].append(np.zeros([len(observations['samples']), len(circenters), patchSize_px, patchSize_px, 2], dtype=np.float32))
        ...
        for nidx, sample in enumerate(tqdm(observations['samples'])):
            ...
            if sample['extrinsics']['usage']:
                for idx, psfboard in enumerate(self.psf_board):
                    ...
                    circenters, patchsize = psfboard['cir2d'].reshape(-1, 3), training_data['patchSize_px'][idx]
                    ...
                    offsets, valid_mask = self.optimizePatches(np.squeeze(aif_patches), blobParams, psfboard['radius'].reshape(-1))
                    ...
                    if valid_mask.any():
                        ...
                        clean_patches = geomath.subpixel_cropper_batch(np.float32(repeat(psfboard['template'], 'h w -> h w 3')), circenters, patchsize, mode='bilinear')
